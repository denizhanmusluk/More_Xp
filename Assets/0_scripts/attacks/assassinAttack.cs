...
public class assassinAttack : MonoBehaviour
{
    ...
    IEnumerator forwardMove()
    {
        for (int i = 0; i < enemies.Count; i++)
        {
            ...
            {
                transform.position = Vector3.MoveTowards(transform.position, enemies[i].transform.position, 50 * Time.deltaTime);
                yield return null;
            }
        ...
        {
            transform.position = Vector3.MoveTowards(transform.position, _playerBeh.transform.position, 50 * Time.deltaTime);
            yield return null;
        }
