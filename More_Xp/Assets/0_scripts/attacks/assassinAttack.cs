...
public class assassinAttack : MonoBehaviour
{
    ...
    IEnumerator forwardMove()
    {
        for (int i = 0; i < enemies.Count; i++)
        {
            ...
            {
                transform.position = Vector3.MoveTowards(transform.position, enemies[i].transform.position, 400 * Time.deltaTime);
                yield return null;
            }
        ...
        {
            transform.position = Vector3.MoveTowards(transform.position, _playerBeh.transform.position, 400 * Time.deltaTime);
            yield return null;
        }
    ...
    private void OnTriggerStay(Collider other)
    {
        ...
        {
            other.GetComponent<enemy>().dead(Globals.tornadoDamage, 2.3f * (transform.up + (other.transform.position - transform.position).normalized));
